...
export default function OrdersAdminPage() {
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    const matchesSearch =
      ...
      (o.payment_method_name && o.payment_method_name.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const handleExportExcel = () => {
    if (filteredOrders.length === 0) {
      toast.error('Tidak ada data transaksi yang dapat diexport.');
      return;
    }

    const exportData = filteredOrders.map((o, index) => ({
      No: index + 1,
      'Invoice Code': o.invoice_code,
      'Klien Perusahaan': o.company_name,
      'Email Customer': o.customer_email,
      'Kontak Person': o.contact_name || '-',
      'Jenis Order': o.order_type === 'TOPUP_BALANCE' ? 'Top-Up Wallet' : 'Beli Kuota',
      'Metode Pembayaran': o.payment_method_name || 'Transfer Bank Manual',
      'Kode Metode': o.payment_method_code || 'MANUAL_BCA',
      Subtotal: Number(o.subtotal || 0),
      'Biaya Admin': Number(o.fee_amount || 0),
      'Total Tagihan': Number(o.total_amount || 0),
      Status: o.status,
      'Tanggal Transaksi': new Date(o.created_at).toLocaleDateString('id-ID'),
      'Tanggal Lunas': o.paid_at ? new Date(o.paid_at).toLocaleDateString('id-ID') : '-',
    }));

    const worksheet = XLSX.utils.json_to_sheet(exportData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Orders Transaksi');
    XLSX.writeFile(workbook, `Laporan_Transaksi_Orders_${new Date().toISOString().slice(0, 10)}.xlsx`);
    toast.success('File Excel laporan transaksi berhasil diunduh!');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Search & Multi Advanced Filter Controls */}
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm space-y-3">
        <div className="flex flex-col md:flex-row items-center justify-between gap-3">
          <div className="flex flex-wrap items-center space-x-1 bg-slate-100 p-1 rounded-xl w-full md:w-auto">
            {[
              { id: 'ALL', label: 'SEMUA' },
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
            <div className="flex items-center gap-1 text-xs text-slate-500 font-medium">
              <SlidersHorizontal className="w-3.5 h-3.5" /> Jenis Order:
            </div>
            <select
              value={filterType}
              onChange={(e) => {
            ...
            >
              ...
              <option value="TOPUP_BALANCE">Top-Up Wallet</option>
              <option value="DIRECT_QUOTA">Beli Kuota Tes</option>
            </select>
            ...
            <div className="relative w-full md:w-64">
              ...
              <input
                ...
                placeholder="Cari Invoice, Klien, Email..."
                className="pl-9 w-full py-1.5 text-xs border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500"
              />
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <>
                      ...
                      <tr key={o.id} className="hover:bg-slate-50 transition-all">
                        ...
                        <td className="py-3.5 px-4 font-medium text-slate-800">
                          <div className="font-bold flex items-center gap-1">
                            ...
                            {o.payment_method_name || 'Transfer Bank BCA'}
                          </div>
                          <span className="text-[10px] text-slate-400 font-mono">
                            {o.payment_method_code || 'MANUAL_BCA'}
                          </span>
                        </td>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-600">
              <div>
                Menampilkan <strong className="text-slate-900">{filteredOrders.length > 0 ? startIndex + 1 : 0}</strong> -{' '}
                <strong className="text-slate-900">{Math.min(startIndex + pageSize, filteredOrders.length)}</strong> dari{' '}
                <strong className="text-slate-900">{filteredOrders.length}</strong> transaksi
              </div>
