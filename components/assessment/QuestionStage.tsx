    ...
    <div className="flex flex-col max-w-2xl w-full animate-fadeUp">
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 md:p-10 min-h-[420px] flex flex-col justify-center">
        ...
        <div className="text-center mb-8">
          {isDisc && (
            <span className="inline-block px-3 py-1 bg-indigo-50 text-indigo-700 font-bold text-[11px] uppercase tracking-wider rounded-full mb-2">
        ...
        {isDisc ? (
          <div className="space-y-3 w-full">
            ...
            {!isDiscValid && (
              <div className="mt-4 flex items-center justify-center gap-2 text-amber-600 text-xs font-semibold bg-amber-50 py-2.5 px-4 rounded-xl border border-amber-200">
          ...
          questionType === 'essay' || questionType === 'short_answer' ? (
            <div className="w-full space-y-3">
              <textarea
                ...
                className="w-full p-4 border border-slate-300 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800"
              />
              {!isNonDiscValid && (
                <div className="flex items-center justify-center gap-2 text-rose-600 text-xs font-semibold bg-rose-50 py-2 px-4 rounded-xl border border-rose-200">
                  <AlertCircle className="w-4 h-4 shrink-0" />
            ...
            <div className="space-y-3.5 w-full">
              {options.map((opt: string, idx: number) => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 md:p-5 rounded-2xl border-2 cursor-pointer transition-all flex items-center justify-between font-medium text-slate-700 hover:border-slate-300 ${
                      isSelected ? 'shadow-sm font-semibold' : 'bg-white border-slate-200'
                    }`}
                    style={
                      isSelected
                    ...
                    <div className="flex items-center gap-3">
                      <span
                        className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 border ${
                          isSelected ? 'text-white border-transparent' : 'bg-slate-100 text-slate-500 border-slate-200'
                        }`}
              ...
              {!isNonDiscValid && (
                <div className="mt-4 flex items-center justify-center gap-2 text-rose-600 text-xs font-semibold bg-rose-50 py-2.5 px-4 rounded-xl border border-rose-200">
                  <AlertCircle className="w-4 h-4 shrink-0" />
      ...
      <div className="flex items-center justify-between mt-8 gap-4 px-2">
        <button
          onClick={onPrev}
          disabled={currentQ === 0}
          className="flex items-center gap-2 text-slate-500 font-bold text-sm px-6 py-3 rounded-2xl border border-slate-200 bg-white hover:bg-slate-50 hover:text-slate-700 transition-all disabled:opacity-30 disabled:pointer-events-none"
        >
        ...
        <button
          ...
          className="flex items-center gap-2 text-white font-bold text-sm px-10 py-3 rounded-2xl shadow-lg transition-all hover:opacity-90 active:scale-95 disabled:opacity-40 disabled:pointer-events-none"
          style={{ backgroundColor: brandColor }}
        >
          {currentQ === questions.length - 1 ? 'Selesai' : 'Lanjut'} <ChevronRight className="w-5 h-5" />
        </button>
      </div>
    </div>
  );
